...
rejectionRates = [0.1, 0.15, 0.20, 0.30];
...
%**********print to file*********************
for i = 1: length(rejectionRates)
    rejectionRate_thr = rejectionRates(i);
    
    logfile = strcat('log/', dataset, '_alpha_', num2str(rejectionRate_thr), '_time_', char(datetime('now')), '.txt');
    fprintf(logfile);
    fid = fopen(logfile,'wt');
    
    %*******Directories*******************
    
    img_dir = strcat('image/', dataset);       % directory for the image database
    data_dir = strcat('data/', dataset);       % directory for saving SIFT descriptors
    Bpath = [strcat('dictionary/dictionary_1000_', dataset, '.mat')];
    bow_fea_dir = strcat('gen_feature_matlab/bow/', dataset);    % directory for saving BoW features
    cnn_fea_dir = strcat('gen_feature_matlab/cnn/', dataset);
    
    %**********Record setting**************
    fprintf(fid, 'image dir: %s\n', img_dir);
    fprintf(fid, 'rejection rate: %.4f\n', rejectionRate_thr);
    ...
    [total_data_s1, total_label_s1, nclass_s1, clabel_s1, fdatabase_s1] = load_bow_features(database, bow_fea_dir, genFeature, genData, Bpath, dataset);
    [total_data_s2, total_label_s2, nclass_s2, clabel_s2, fdatabase_s2] = load_cnn_features(database, cnn_fea_dir, dataset);
    
    fprintf (fid, 'nclass1, nclass2 %d %d \n', nclass_s1, nclass_s2);
    ...
    for ii = 1:nRounds
        ...
        [t_opt, SVM_model] = Build_RO_SVM(total_data_s1, total_label_s1, tr_idx, ts_idx, tr_without_v_idx, v_idx, nclass,rejectionRate_thr, fid);
        
        [reject_index, stage1_predict_ts] = Stage1_Classification(t_opt,SVM_model,total_data_s1, total_label_s1, tr_idx, ts_idx,nclass, fid);
        
        total_label = total_label_s1;
        
        acc_s1 = length(find(stage1_predict_ts == total_label(ts_idx)))/length(total_label(ts_idx));
        fprintf(fid, 'Stage 1 Round: %d, Accuracy: %.4f\n', ii, acc_s1);
        
        [stage2_predict_ts, stage2_predict_reject] = Stage2_Classification(total_data_s2, total_label_s2, tr_idx, ts_idx, reject_index, fid);
        acc_s2 = length(find(stage2_predict_ts == total_label(ts_idx)))/length(total_label(ts_idx));
        fprintf(fid, 'Stage 2 Round: %d, Accuracy: %.4f\n', ii, acc_s2);
        ...
        fprintf(fid, '2 stage Round: %d, Accuracy: %.4f\n', ii, two_stage_acc);
        
        %     fprintf(fid, 'Round: %d, stage 1 predict ts: %d\n', ii, stage1_predict_ts);
        %     fprintf(fid, 'Round: %d, stage 2 predict ts: %d\n', ii, stage2_predict_ts);
        %     fprintf(fid, 'Round: %d, reject index: %d\n', ii, reject_index);
        %     fprintf(fid, 'Round: %d, stage 2 predict reject: %d\n', ii, stage2_predict_reject);
        %     fprintf(fid, 'Round: %d, 2 stage predict ts: %d\n', ii,two_stage_predict_ts);
    ...
    fprintf(fid, 'Stage 1 Avg, std acc %.4f %.4f \n',avg_acc_s1, std_acc_s1);
    fprintf(fid, 'Stage 2 Avg, std acc %.4f %.4f \n',avg_acc_s2, std_acc_s2);
    fprintf(fid, '2 stage Avg, std acc %.4f %.4f \n',avg_acc_2_stage, std_acc_2_stage);
    fid = fclose(fid);
end
